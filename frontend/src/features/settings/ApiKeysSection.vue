...
<template>
  ...
  <section>
    <p class="mb-3 px-1 text-xs text-secondary">
      ...
      without your password. Keys get the same access a logged-in session has, except for account
      security actions.
    ...
    <div class="rounded-card border border-neutral-200 bg-white px-4 py-4">
      ...
      <p v-if="createError" class="mt-2 text-xs text-coral-600">{{ createError }}</p>
    </div>

    <!-- Existing keys -->
    <div v-if="loading" class="mt-3 px-4 py-6 text-center text-sm text-secondary">Loading…</div>
    <div v-else-if="keys.length > 0" class="mt-3 rounded-card border border-neutral-200 bg-white">
      <ul class="divide-y divide-neutral-100">
        <li v-for="key in keys" :key="key.id" class="flex items-start justify-between px-4 py-3">
          <div>
            ...
            <p class="mt-0.5 text-xs text-secondary">
              ...
              <template v-if="key.expiresAt"> · Expires {{ mediumDate(key.expiresAt) }} </template>
          ...
          <span v-if="key.revokedAt" class="mt-0.5 flex-none text-xs text-secondary">
            Revoked
          </span>
          <button
            v-else
