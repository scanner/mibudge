...
def _fetch_existing(
    ...
    represent genuinely distinct same-day, same-amount, same-description
    transactions (e.g. two coffee purchases).  During import the list is
    consumed (popped) so that each server transaction is matched to at
    most one import row.
    ...
    Returns:
        A mapping from (date, amount, description) to list of
        (transaction_id, transaction_type) pairs.
    """
...
def import_statement(
    ...
    *,
    ...
    """
    Import transactions from an already-parsed bank statement.
    ...
    for i, tx in enumerate(transactions):
        ...
        if matches:
            # Pop one match so each server-side transaction is consumed
            # at most once.  This handles the "two coffees" case: if the
            # source has 2 entries and the server has 2, both are skipped;
            # if the server has 1, the second row is imported.
            tx_id, existing_type = matches.pop(0)
            # Backfill transaction_type when the server has it empty but
            # the parser has since learned to classify this description.
